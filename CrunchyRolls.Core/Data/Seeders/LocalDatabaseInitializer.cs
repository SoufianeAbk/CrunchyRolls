...
namespace CrunchyRolls.Core.Data.Seeders
{
    /// <summary>
    /// Initialiseert en seeded lokale SQLite database
    ...
    public static class LocalDatabaseInitializer
    {
        ...
        public static async Task InitializeAsync()
        {
            try
            {
                ...
                using (var context = new LocalDbContext())
                {
                    ...
                    Debug.WriteLine("✅ Database created or already exists");
                    ...
                    {
                        Debug.WriteLine("ℹ️ Database already contains data");
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Seed categories and products (from API later)
        /// </summary>
        private static async Task SeedDatabaseAsync(LocalDbContext context)
        {
            try
            {
                // Add categories
                ...
                Debug.WriteLine($"✅ Seeded {categories.Count} categories");

                // Add products
                var products = GetSeedProducts();
                await context.Products.AddRangeAsync(products);
                await context.SaveChangesAsync();
                Debug.WriteLine($"✅ Seeded {products.Count} products");
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Clear local database (useful for testing/debugging)
        /// </summary>
        public static async Task ClearDatabaseAsync()
        {
            try
            {
                Debug.WriteLine("🗑️ Clearing local database...");
